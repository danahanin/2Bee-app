...
class AuthManager:
    ...
    def get_authorization_url(self, consent_id: str) -> str:
        ...
        """
        try:
            response = self.client.get(f"/api/v1/consents/{consent_id}/authorize")
    ...
    def exchange_code_for_tokens(
        ...
        try:
            response = self.client.post("/api/v1/oauth/token", data=data)
            return {
    ...
    def refresh_access_token(self, refresh_token: str) -> Dict:
        ...
        try:
            ...
            return {
                "access_token": response.get("access_token"),
                "refresh_token": response.get("refresh_token", refresh_token),  # May get new refresh token
                "expires_in": response.get("expires_in", 3600),
                "token_type": response.get("token_type", "Bearer")
            }
        except OpenFinanceAPIError as e:
    ...
    def revoke_token(self, token: str, token_type: str = "access_token") -> bool:
        ...
        try:
            self.client.post("/api/v1/oauth/revoke", data=data)
            return True
        except OpenFinanceAPIError as e:
